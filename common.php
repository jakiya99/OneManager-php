...
-->';
    ...
    if (substr($theme,-4)=='.php') {
        ...
        if ($_SERVER['ishidden']==4) {
            ...
            if (!getConfig('dontBasicAuth')) {
                // use Basic Auth
                return output('Wrong Password😐 ,Password එක වැරදි :/  හරියට ගහන්න 😒 password= <font color='red'>/nocb*|$</font> ✅ <a href="https://t.me/elearnlkbot">Support</a>', 401, ['WWW-Authenticate'=>'Basic realm="Secure Area"']);
            }
            /*$tmp[1] = 'a';
            while ($tmp[1]!='') {
                $tmp = splitfirst($html, '<!--ListStart-->');
