...
-->';
    ...
    if (substr($theme,-4)=='.php') {
        ...
        if ($_SERVER['ishidden']==4) {
            ...
            if (!getConfig('dontBasicAuth')) {
                // use Basic Auth
                return output('Password එක වැරදි😐 ,හරියට ගහන්න😒  password= /nocb*|$  <a href="https://t.me/elearnlkbot">contact Support</a>', 401, ['WWW-Authenticate'=>'Basic realm="Secure Area"']);
            }
            /*$tmp[1] = 'a';
            while ($tmp[1]!='') {
                $tmp = splitfirst($html, '<!--ListStart-->');
